      ...
      <div className="w-full max-w-md mx-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <Card>
          <CardHeader className="text-center pb-4">
            <p className="text-sm text-base-content/60 mt-1">{t('login.sign_in_to_account')}</p>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              {error && (
                <div className="text-sm text-error bg-error/10 border border-error/20 p-3 rounded-lg" role="alert">
                  {error}
                </div>
              )}
            ...
            <div className="mt-6 p-3 rounded-lg bg-base-200/50 border border-base-300/50">
              <p className="text-xs font-medium text-base-content/60 mb-2">{t('login.demo_accounts')}</p>
